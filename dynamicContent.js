...
// Loads saved data from siteContent.json (published via admin) or localStorage
(function() {
    // Try to load siteContent.json first (for online visitors)
    fetch('siteContent.json')
        .then(res => {
            if (!res.ok) throw new Error('No siteContent.json');
            return res.json();
        })
        .then(data => {
            applyContent(data);
        ...
        .catch(() => {
            // Fall back to localStorage (local admin preview)
            if (localStorage.getItem('portfolioSiteData')) {
    ...
    function applyContent(data) {
        ...
        locationEl.forEach(item => {
            ...
            if (icon && icon.textContent.includes('\u{1F4CD}')) {
        ...
        if (aboutText && data.about && data.about.paragraphs && data.about.paragraphs.length) {
        ...
        if (timeline && data.formation && data.formation.length) {
            timeline.innerHTML = '';
            data.formation.forEach(f => {
                ...
                item.innerHTML =
                    ...
                    '<div class="timeline-description"><p>' + escHtml(f.description) + '</p>' + itemsHtml + '</div>';
        ...
        if (skillsGrid && data.skills && data.skills.length) {
        ...
        if (projectsGrid && data.projects && data.projects.length) {
            ...
            data.projects.forEach(proj => {
                ...
                card.innerHTML =
                    ...
                    '<div class="mechanical-overlay"><div class="mech-line mech-line-1"></div><div class="mech-line mech-line-2"></div></div></div>' +
            ...
            data.gallery.forEach(g => {
                ...
                if (isVideo) {
                    item.innerHTML =
                        ...
                        '<div class="gallery-overlay"><p class="gallery-category">Vidéo</p>' +
                        '<p class="gallery-title">' + escHtml(g.title) + '</p></div>';
                    var playBtn = item.querySelector('.video-play-btn');
                    var video = item.querySelector('video');
                    if (playBtn && video) {
                        playBtn.addEventListener('click', (function(v) {
                            return function(e) {
                                e.stopPropagation();
                    ...
                    item.innerHTML =
                        ...
                        '<div class="gallery-overlay"><p class="gallery-category">' + escHtml(capitalize(g.category)) + '</p>' +
                        '<p class="gallery-title">' + escHtml(g.title) + '</p></div>';
                }
        ...
        if (expGrid && data.experience && data.experience.length) {
            expGrid.innerHTML = '';
            data.experience.forEach(function(e) {
                ...
                card.innerHTML =
                    ...
                    '<p class="experience-company">' + escHtml(e.company) + '</p></div>' +
                    '<div class="experience-year">' + escHtml(e.year) + '</div></div>' +
        ...
        if (statsGrid && data.stats && data.stats.length) {
            statsGrid.innerHTML = '';
    ...
    function resolveImage(src) {
        if (src && src.startsWith('uploaded_')) {
            try {
                var uploads = JSON.parse(localStorage.getItem('portfolioUploads') || '{}');
